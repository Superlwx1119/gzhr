<template>
  <!-- 居民政策信息新增-->
  <form-dialog title="居民政策信息新增" width="1000px" new-class="big" :is-show="visible" @update:isShow="updateIsShow">
    <FormItems ref="form" :is-grid="false" :items-datas="itemsDatas" :model="addForm" :rules="rules" :form-datas="addForm">
      <template slot="医保区划1">
        <insurance-area v-model="addForm.医保区划1" />
      </template>
      <template slot="险种1">
        <insurance-type v-model="addForm.险种1" />
      </template>
      <template slot="人员类别1">
        <personal-type v-model="addForm.人员类别1" />
      </template>
    </FormItems>
