<template>
  ...
  >
    <div class="box">
      <div class="box-body">
        <table class="table-data total-infoExit">
          <tbody>
            <tr>
              <td>现工作单位</td>
            ...
            <tr>
              ...
              <td>进入单位时间</td>
              <td>{{ infoData.rc0301 }}</td>
              <td>退出时人员类别</td>
              <td>{{ infoData.rc0215 }}</td>
            </tr>
            <tr>
              <td>退出时任职务</td>
              <td>{{ infoData.rc0704 }}</td>
              <td>退出时岗位等级</td>
              <td>{{ infoData.rc0703 }}</td>
              <td>退出方式</td>
              <td>{{ infoData.rc0909 }}</td>
            </tr>
            <tr>
              <td>退出日期</td>
              <td>{{ infoData.rc0502 }}</td>
              <td>去往何处</td>
              <td colspan="3">{{ infoData.rc0503 }}</td>
            </tr>
            <tr>
              <td>批准文号</td>
              <td colspan="5">{{ infoData.rc0506 }}</td>

            </tr>
            <tr>
              <td>备注</td>
              <td colspan="5">{{ infoData.rc0504 }}</td>
            </tr>
          </tbody>
        </table>
      </div>
...
<script>
import { infoExit, retireAudit } from '@/api/BaseInformation/PersonalInformationManagement/exit'
...
export default {
  ...
  methods: {
    ...
    infoExit(row) {
      this.loading = true
      infoExit({ arc081: row.arc081 }).then(res => {
        this.loading = false
        if (res.code === 0) {
