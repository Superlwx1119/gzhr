  ...
  <div class="specialPersonBonusVerification">
    <normal-layer :search-number="6">
      ...
      <div slot="table-title" class="box-header handle">
        <span class="box-title">退出管理列表</span>
    ...
    <!-- 新增 -->
    <AddDialog ref="add" v-model="isShowAdd" :detail-info="detailInfo" dialog-title="选择需要退出人员" @search="search" />
    <!-- 修改 -->
    <EditDialog ref="edit" v-model="isShowEdit" :operation="operation" :detail-info="detailInfo" dialog-title="人员退出登记" @cloeseParent="cloeseParent" />
    <!-- 查看 -->
    <DetailDialog ref="detail" v-model="isShowDetail" :operation="operation" :detail-info="detailInfo" dialog-title="人员退出详情" />
    <!-- 明细 -->
...
<script>
import { exitList, deleteExit } from '@/api/BaseInformation/PersonalInformationManagement/exit'
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        { label: '退出方式', prop: 'rc0501' },
        { label: '调往单位名称', prop: 'rc0503' },
        { label: '状态位', prop: 'i' },
  ...
  methods: {
    showDialog(type, row) {
      ...
      if (type === 'add') {
        this.isShowAdd = true
        this.$refs.add.canExitPersonsList()
      } else if (type === 'detail') {
        this.$refs.detail.infoExit(row)
    ...
    deleteRow(row) {
      this.$msgConfirm('确认删除?', '提示', {
        ...
        deleteExit({ arc061: Number(row.arc061) }).then(res => {
