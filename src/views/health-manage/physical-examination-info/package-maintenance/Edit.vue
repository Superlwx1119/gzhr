<template>
  <!--   体检套餐修改-->
  <form-dialog title="体检套餐修改" width="1000px" new-class="big" :is-show="visible" @update:isShow="updateIsShow">
    <FormItems ref="editForm" :items-datas="itemsDatas" :model="editForm" :rules="rules" :form-datas="editForm">
      <template slot="体检类别1">
        <health-type v-model="editForm.体检类别1" />
      </template>
      <template slot="套餐性别1">
        <package-sex v-model="editForm.套餐性别1" />
      </template>
      <template slot="套餐年龄段1">
        <age-range v-model="editForm.套餐年龄段1" />
      </template>
      <template slot="默认套餐1">
        <el-select v-model="editForm.默认套餐1" style="width: 100%;" placeholder="请选择">
...
export default {
  ...
  methods: {
    ...
    cancel() {
      this.visible = false
      this.$refs['editForm']['elForm'].clearValidate()
