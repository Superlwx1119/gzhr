<template>
  ...
  <div class="disposeManagement">
      ...
      <template>
        <my-table-view v-loading="loading" :border="true" :max-cloumns="20" :columns="columns" :data="tableData">
          ...
          <template slot="operation" slot-scope="scope">
            <el-button type="text" @click="repeal(scope.row)">撤销</el-button>
          </template>
        </my-table-view>
    ...
    <repeal-dialog v-model="showRepealDialog" :dialog-title="dialogTitle" />
  </div>
</template>
...
import RepealDialog from './repeal'
export default {
  name: 'DisposeManagement',
  components: { FormItems, NormalLayer, OrganizationName, PunishmentType, ApprovalStatus, OrganizationInfoView, PersonnelInfoView, RepealDialog },
  mixins: [pageHandle],
  props: {},
  data() {
    return {
  ...
  methods: {
    ...
    repeal() {
      this.dialogTitle = '查看处分信息'
      this.showRepealDialog = true
    },
    handleSave() {
