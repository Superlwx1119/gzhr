<template>
  <form-dialog title="政策信息新增" :is-show="visible" width="1000px" new-class="big" @update:isShow="updateIsShow">
    <form-items
      ref="addForm"
      :model="addForm"
      :rules="rules"
    ...
    >
      <template slot="tjys">
        <div>
          <el-input v-model="addForm.tjys" placeholder="请输入内容" class="input-with-select">
      ...
      <template slot="jsys">
        <div>
          <el-input v-model="addForm.jsys" placeholder="请输入内容" class="input-with-select">
      ...
      <template slot="blys">
        <div>
          <el-input v-model="addForm.blys" placeholder="请输入内容" class="input-with-select">
...
export default {
  ...
  methods: {
    ...
    selectCardinality() {
      this.addForm.jsys = '个人工资【社平60%保底300%拦头】'
    },
    selectConditional() {
      this.addForm.tjys = '基本医疗单位缴纳'
    },
    selectRatio() {
      this.addForm.blys = '单位缴纳8%'
    }
  }
}
</script>
