...
<template>
  <div class="third-index supplementaryReceipt">
    ...
    <!-- 审核 -->
    <AuditDialog v-model="isShowAudit" dialog-title="特殊收款征集" />
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      searchArr: ['手工费用类型', '起止时间', '录入方式', '费用所属期', 'button'],
      activeName: 'first',
      selection: [],
      queryForm: {},
      isShowSingle: false,
      isShowAudit: false,
      businessSelection: [],
      confluenceData: [1, 2, 3, 4, 5, 6, 7, 8, 9].map(item => {
        return {
          aa: 'CN00' + item,
      ...
      confluenceColumns: [
        { type: 'selection' },
        { type: 'index', label: '序号' },
        { label: '款项类型', type: 'custom', slotName: '款项类型', prop: 'aa' },
        { label: '款项金额', type: 'custom', slotName: '款项金额', prop: 'bb' },
        { label: '备注', type: 'custom', slotName: '备注', prop: 'cc' },
